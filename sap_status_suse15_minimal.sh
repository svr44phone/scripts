#!/usr/bin/env bash
# sap_status_suse15_minimal.sh
# Purpose: Minimal SAP status check for SUSE 15
# Outputs: Hostname, SID, Nr, Running (Yes/No)
# Supports: Clustered HANA, standalone HANA, ENSA2 ASCS/ERS clusters, standalone ASCS/ERS, APP servers
...
# Capture crm_mon output for cluster detection
CRM_OUT=""
if systemctl is-active --quiet pacemaker; then
  CRM_OUT=$(crm_mon -r -1 2>/dev/null)
fi

# Track printed instances to avoid duplicates
