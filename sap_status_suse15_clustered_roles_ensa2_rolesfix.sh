#!/usr/bin/env bash
# sap_status_suse15_clustered_roles_ensa2_rolesfix.sh
# Purpose: Detect SAP instance status on SUSE Linux Enterprise Server 15 with Pacemaker clusters
# Adds ENSA2 resource name handling and full hana_role attribute parsing for accurate HANA SR role detection
# Usage: ./sap_status_suse15_clustered_roles_ensa2_rolesfix.sh [--debug] [--owner]
...
# Capture crm_mon output once for parsing
CRM_OUT=$(crm_mon -r -1 2>/dev/null)

# Detect Pacemaker cluster
if systemctl is-active --quiet pacemaker && [[ -n "$CRM_OUT" ]]; then
  ...
  echo "$CRM_OUT" | awk '/SAPInstance|SAPHana/{print}' | while read -r line; do
    ...
    # Match ENSA2 ASCS/ERS resource names
    if [[ "$res_name" =~ SAPInstance_([A-Z0-9]+)_ASCS([0-9]+) ]]; then
      ...
      NR="00"
      TYPE="HANA"
      # Full attribute parsing for hana_role
      ROLE=$(echo "$CRM_OUT" | awk -v sid=$SID '/SAPHanaController/ && $0 ~ sid {print}' | grep -o 'hana_role=[^ ]*' | cut -d= -f2 | head -n1)
      if [[ -z "$ROLE" ]]; then
        ROLE=$(echo "$CRM_OUT" | awk -v sid=$SID '/SAPHanaController/ && $0 ~ sid {print}' | grep -o 'PRIMARY\|SECONDARY' | head -n1)
      fi
      if [[ -z "$ROLE" ]]; then
        ROLE=$(echo "$CRM_OUT" | awk -v sid=$SID '/SAPHanaController/ && $0 ~ sid {print}' | grep -o 'Master\|Slave' | head -n1)
    ...
    if [[ -n "$PROFILE" ]]; then
      ...
      if [[ "$RUNNING" == "No" ]]; then
        if [[ -n "$SERVICE_NAME" && $(systemctl is-active "$SERVICE_NAME" 2>/dev/null) == "active" ]]; then
          RUNNING="Yes"
        elif pgrep -f "sapstartsrv.*$PROFILE" >/dev/null 2>&1; then
          RUNNING="Yes"
        fi
      fi
      printf "%-15s %-6s %-4s %-8s %-10s %-12s" "$HOST" "$SID" "$NR" "$TYPE" "$RUNNING" "$ROLE"
...
for sid_dir in /usr/sap/*/*/exe; do
  ...
  [[ "$SC_OUT" =~ GREEN|Running ]] && RUNNING="Yes"
  if [[ "$RUNNING" == "No" ]] && pgrep -f "sapstartsrv.*$possible_profile" >/dev/null 2>&1; then
    RUNNING="Yes"
  fi
