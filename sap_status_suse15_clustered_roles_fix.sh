#!/usr/bin/env bash
# sap_status_suse15_clustered_roles_fix.sh
# Purpose: Detect SAP instance status on SUSE Linux Enterprise Server 15 with Pacemaker clusters
# Adds HANA SR role mapping and robust APP fallback detection
# Usage: ./sap_status_suse15_clustered_roles_fix.sh [--debug] [--owner]
...
if [[ -f /usr/sap/sapservices ]]; then
  while IFS= read -r line; do
    ...
    if [[ -n "$PROFILE" ]]; then
      ...
      # Fallback for APP servers if sapcontrol fails
      if [[ "$RUNNING" == "No" ]]; then
