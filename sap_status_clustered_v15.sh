...
if command -v SAPHanaSR-showAttr >/dev/null 2>&1; then
    SAPHANA_OUTPUT=$(SAPHanaSR-showAttr 2>/dev/null)
    dbg "Captured SAPHanaSR-showAttr output"
    while read -r line; do
        [[ "$line" =~ ^([a-zA-Z0-9_-]+)[[:space:]]+([A-Z]+) ]] || continue
        HANA_HOST=${BASH_REMATCH[1]}
        HANA_STATE=${BASH_REMATCH[2]}
        HANA_SR["$HANA_HOST"]="$HANA_STATE"
    done <<< "$SAPHANA_OUTPUT"
fi
...
grep -v '^#' "$SAPSERVICES" | grep "/usr/sap/" | while read -r line; do
    ...
    if [[ -x "$SAPCONTROL" ]]; then
        ...
        if [[ "$INSTANCE_TYPE" =~ ^SMDA ]]; then
            ...
            ROLE="$INSTANCE_TYPE"
            # Override Running=No if this host is not the owner
            if [[ "$SHOW_OWNER" -eq 1 ]] && [[ "${OWNER_MAP[$SID]:-}" != "$HOSTNAME" ]]; then
                RUNNING="No"
                OWNER="${OWNER_MAP[$SID]:-$HOSTNAME}"
            else
                # Node owns resource, check sapcontrol/pgrep
                ...
                if [[ $DISP_GREEN -gt 0 && $DIA_GREEN -gt 0 ]]; then
                    ...
                    if pgrep -u "$SIDADM" -f "disp|enq|sapstartsrv" >/dev/null 2>&1; then
                        RUNNING="Yes"
                    else
                        RUNNING="No"
                    fi
    ...
    # Owner from Pacemaker if requested
